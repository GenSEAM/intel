(module asl-intel/xref-test
  ...
  :x [test-parse-ref-uri-task
      test-parse-ref-uri-adr-and-c
      test-parse-ref-uri-sym-and-chunk
      test-parse-ref-uri-file-and-format
      test-extract-docstring-refs
      test-xref-graph-bidirectional-and-provenance
      test-health-integrity-and-cycle-detection
      run-tests]
  :i [(xref :a xr)
      (health :a h)
      (mem/src/xref :a mxr)])
...
(df test-parse-ref-uri-adr-and-c [] -> Bool
  :d "Tests parsing of ADR and constraint invariant reference URIs"
  (let [(t-adr (xr/parse-ref-uri "adr:d-0012"))
        (t-d (xr/parse-ref-uri "d:d-0012"))
        (t-c (xr/parse-ref-uri "c:c-0001"))]
    (assert (= (.-kind t-adr) "adr") "ADR URI kind must equal 'adr'")
    (assert (= (.-id t-adr) "d-0012") "ADR URI id must equal 'd-0012'")
    (assert (= (.-kind t-d) "adr") "d: shortcode URI kind must equal 'adr'")
    (assert (= (.-id t-d) "d-0012") "d: shortcode URI id must equal 'd-0012'")
    (assert (= (.-kind t-c) "c") "c: constraint URI kind must equal 'c'")
    (assert (= (.-id t-c) "c-0001") "c: constraint URI id must equal 'c-0001'")
    true))

(df test-parse-ref-uri-sym-and-chunk [] -> Bool
...
(df test-parse-ref-uri-file-and-format [] -> Bool
...
(df test-extract-docstring-refs [] -> Bool
  :d "Tests extraction of @ref: annotations embedded in ASL docstrings"
  (let [(doc "Formats model request payload. @ref:task:task-325-1 @ref:adr:d-0012 @ref:c:c-0001")
    ...
    (assert (= (.-id (.-target r2)) "d-0012") "Second ref target id must equal 'd-0012'")
    (assert (= (.-kind (.-target r3)) "c") "Third ref target kind must equal 'c'")
    (assert (= (.-id (.-target r3)) "c-0001") "Third ref target id must equal 'c-0001'")
    true))

(df test-xref-graph-bidirectional-and-provenance [] -> Bool
  ...
  (let [(g0 (mxr/make-xref-graph))
        ...
        (g7 (mxr/xref-add-edge g6 "sym:asl-harness/ModelOptions" "adr:d-0012" "provenance"))
        (g8 (mxr/xref-add-edge g7 "sym:asl-harness/ModelOptions" "c:c-0001" "provenance"))
    ...
    (assert (list-contains? prov "adr:d-0012") "Provenance must include adr:d-0012")
    (assert (list-contains? prov "c:c-0001") "Provenance must include c:c-0001")
    true))

(df test-health-integrity-and-cycle-detection [] -> Bool
  ...
  (let [(doc-broken "@ref:task:task-ghost-999")
        ...
        (valid-targets (list "task:task-325-1" "task:task-325-2" "adr:d-0012"))
...
(df run-tests [] -> Bool
  ...
  (let [(_t1 (test-parse-ref-uri-task))
        (_t2 (test-parse-ref-uri-adr-and-c))
        (_t3 (test-parse-ref-uri-sym-and-chunk))
        (_t4 (test-parse-ref-uri-file-and-format))
        (_t5 (test-extract-docstring-refs))
        (_t6 (test-xref-graph-bidirectional-and-provenance))
        (_t7 (test-health-integrity-and-cycle-detection))]
    true))
